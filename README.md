...
-------
###Commands
Commands are now also much easier. An echo command can be made in one line without use of other external libraries (such as google guava's `Joiner` class). In Kotlin, we can just echo arguments back to the sender with
```
...
Also, Kotlin has safe casting as well. This makes runinng player-specific funtions on a `CommandSender` very easy!
```
